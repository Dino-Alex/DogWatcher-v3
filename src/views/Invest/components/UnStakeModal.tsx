...
const UnStakeModalPoolStore: React.FC<UnStakeModalProps> = ({onDismiss, title, totalStaked, onRefresh, contractAddress}) => {
    ...
    const { handleUnStaked, requestedUnStake, pendingUnStaked } = useUnStaked(stakeAmount, contractAddress, onRefresh)
    return (
        <CustomModal title="" onDismiss={onDismiss}>
            <Container>
                ...
                <ConfirmButton 
                    disabled={pendingUnStaked || Number(stakeAmount) === 0 || Number(stakeAmount) > totalStaked}
